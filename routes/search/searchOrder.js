...
// Route to search for orders by orderNumber
router.post("/order", (req, res) => {
  const { searchTerm } = req.body; // Extract search term from request body

  // SQL query to search for orders where orderNumber matches the search term
  const query = "SELECT * FROM `order` WHERE orderNumber LIKE ?";
