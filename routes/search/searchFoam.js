...
// Route to search for foam by foamId
router.post("/foam", (req, res) => {
  const { searchTerm } = req.body; // Extract search term from request body

  // SQL query to search for foam with foamId matching the search term
  const query = "SELECT * FROM foam WHERE foamId LIKE ?";
