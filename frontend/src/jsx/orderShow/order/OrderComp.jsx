...
//   return `${year}-${month}-${day}`; // שינוי סדר הפורמט
// };
...
//   // render supplier details if existed
...
useEffect(() => {
 ...
 const fetchOrderDetails = async (quotationId) => {
  // Check if quotationId is defined before making the request
  if (!quotationId) {
    console.log('Quotation ID is undefined or invalid, skipping the request.');
    return;  // Exit early if quotationId is invalid
  }

  try {
    const response = await axios.get(`http://localhost:3001/orderByQuotation/${quotationId}`);
    // Handle the successful response
    console.log(response.data);
  } catch (error) {
    // Suppress the error if it's a 404 or handle silently
